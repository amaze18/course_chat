...
def get_indexed_course_list(repo_name+index_folder_name/ local_path_to_index_folder):
    '''
    Given repo name, it should return a list of all the course name /folder names which hare in the folder "index".
    '''
    pass
...
def main():
    ...
    #1. LOGIN
    course_name = st.text_input("Enter course name:")
    #2. COURSE CREATION / COURSE UPDATE & UPLOAD FILES & NEW INDEX GENERATION
    if action == "Create New Course":
        create_new_course(course_name)
    elif action == "Upload Files":
        upload_files(course_name)
    #3. UPDATE THE COURSE LIST & GET DROPDOWN
    '''
    indexed_course_list = get_indexed_course_list(local_folder_path)
    c_option = st.selectbox(
     'Which course would you like to learn today ?',
     tuple(indexed_course_list))
    ...
    indexPath = f"{loacl_index_folder_path}/{c_option}"
    '''
    #4. MOVE TO CHATTING WITH THE SELECTED COURSE
    '''
